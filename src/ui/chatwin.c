...
chatwin_incoming_msg(ProfChatWin* chatwin, ProfMessage* message, gboolean win_created)
{
    ...
    auto_char char* new_plain = strdup(message->plain);
    plugins_pre_chat_message_display(message->from_jid->barejid, message->from_jid->resourcepart, new_plain);
                     ...
                     gboolean request_receipt, const char* const replace_id)
{
    ...
    auto_char char* display_message = strdup(message);
    plugins_pre_chat_message_display(myjid->barejid, myjid->resourcepart, display_message);
...
_chatwin_history(ProfChatWin* chatwin, const char* const contact_barejid)
{
    if (!chatwin->history_shown) {
        ...
        while (curr) {
            ...
            plugins_pre_chat_message_display(msg->from_jid->barejid, msg->from_jid->resourcepart, msg->plain);
